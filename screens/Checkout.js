...
import { Header, ProductContainer } from "../components/styles";
            ...
            ListFooterComponent={
              <View style={styles.footerContainer}>
                    ...
                    <View>
                      {shippingLocation ? (
                        <>
                          ...
                          <Text style={styles.sectionText}>
                            {shippingLocation.house}
                          </Text>
                          <Text style={styles.sectionText}>
                            {shippingLocation.fullAddress}
                          </Text>
                        </>
                      ) : (
                        <>
                ...
                <View style={styles.deliverySection}>
                  ...
                  <Text style={styles.sectionText}>
                    LOCAL STANDARD SHIPPING: ₱58
                  </Text>
                  <Text style={styles.deliveryNote}>
                    Order placed now are expected to be delivered within 2-3
                    days
                  </Text>
                </View>
              </View>
              ...
              <View style={styles.popupOverlay}>
                <View style={styles.popupBox}>
                  <Text style={styles.popupText}>
                    You’re about to place your order. Proceed to checkout?
                  </Text>
                        ...
                        try {
                          ...
                          for (const groupedItem of groupedItemsMap.values()) {
                            ...
                            const orderData = {
                              ...
                              address: `${shippingLocation.house || ""}, ${
                                shippingLocation.fullAddress || ""
                              }`,
                            ...
                            if (!orderData.productID) {
                              console.warn(
                                "⚠️ Skipping item with undefined productID:",
                                groupedItem
                              );
                              continue;
                            }
                            ...
                            await addDoc(collection(db, "notifications"), {
                              notifID: `NCK-${Date.now()}-${Math.floor(
                                Math.random() * 1000
                              )}`,
                              userId: customUserId,
                              title: "Order Placed",
                            ...
                            if (productSnap.exists()) {
                              ...
                              const currentQty =
                                currentStock[groupedItem.size] || 0;

                              const newQty = Math.max(
                                currentQty - groupedItem.quantity,
                                0
                              );
                          ...
                          const cartSnap = await getDocs(
                            query(cartRef, where("userId", "==", customUserId))
                          );

                          const deletePromises = cartSnap.docs.map(
                            async (docSnap) => {
