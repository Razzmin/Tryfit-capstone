...
function isBodyInsideBox(lm, x, y, w, h) {
  const left = x;
  const right = x + w;
  const top = y;
  const bottom = y + h;
  ...
  for (let i = 0; i < lm.length; i++) {
    const px = lm[i].x * overlay.width;
    const py = lm[i].y * overlay.height;
...
const w = overlay.width * 0.55;
const h = overlay.height * 0.78;
const x = (overlay.width - w) / 2;
const y = (overlay.height - h) / 2;
const insideBox = isBodyInsideBox(lm, x, y, w, h);

if (!insideBox) {
  drawOverlay(false);
  guidance.innerText = "Move inside the box";
  stillnessCounter = 0;
  lastPose = null;
  return;
}
...
drawOverlay(true);
guidance.innerText = "Perfect distance! Hold still...";

  const movement=poseDifference(lm,lastPose);lastPose=lm;
  if(movement<0.003)stillnessCounter++;else stillnessCounter=0;
  if(stillnessCounter>(TARGET_FPS*1)){stillnessCounter=0;startCountdown(lm);}
}
...
function startCountdown(lm) {
  ...
  function animateNumber(num) {
    countdownEl.innerText = num > 0 ? num : '✔';
  ...
  function updateLoading() {
    ...
    if (progress >= 100) {
      ...
      setTimeout(() => {
        ...
        showCapturedMeasurements(lm); 
      }, 400);
    } else {
      loadingPercent.innerText = `${Math.floor(progress)}%`;
      setTimeout(updateLoading, 120);
    }
  }
  updateLoading();
}

function showCapturedMeasurements(lm) {
  ...
  if(calibrationProvided && !localStorage.getItem('bp_offsets_calibrated')){
    ...
    offsets = calOffsets;
    try { 
      localStorage.setItem('bp_offsets', JSON.stringify(offsets)); 
      localStorage.setItem('bp_offsets_calibrated', '1'); 
    } catch(e){}
  }
...
const passedUid = urlParams.get('uid');        
const passedUserId = urlParams.get('userId'); 
...
saveBtn.addEventListener('click', async () => {
  try {
    ...
    return `MID${datePart}${randomPart}`;
    }
      const data = {
